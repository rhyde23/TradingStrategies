...
class TradingStrategies :
    def __init__(self, strategies) :
        ...
        self.stock_statistics_available = ["MARKET_CAP", "VOLUME", "RELATIVE_VOLUME"]
    ...
    def update_indicators(self) :
        ...
        for period in self.indicator_inputs_required["RSI"] :
            ...
            if difference >= 0 :
                ...
                avg_gain = avg_change_formula(self.indicator_historical[self.ticker][loading_index], period, difference)
                ...
                avg_loss = avg_change_formula(self.indicator_historical[self.ticker][loading_index+1], period, 0)
            ...
            #If the difference is negative (price has decreased since yesterday's close), update the average gain and average loss accordingly using the "avg_change_formula" function
                ...
                #The "avg_gain" float represents the average of all the positive daily changes in closing prices for this RSI period setting
                avg_gain = avg_change_formula(self.indicator_historical[self.ticker][loading_index], period, 0)

                #The "avg_loss" float represents the average of all the negative daily changes in closing prices for this RSI period setting
                avg_loss = avg_change_formula(self.indicator_historical[self.ticker][loading_index+1], period, -difference)
            ...
            loading_index += self.indicator_data_points_needed["RSI"]
        ...
        for period in self.indicator_inputs_required["EMA"] :
            ...
            self.indicators["EMA"][period] = round(ema_formula(self.price, period, self.indicator_historical[self.ticker][loading_index]), 4)

            #Increase the loading index by the number of data points that were used in this specific EMA period setting calculation
            loading_index += self.indicator_data_points_needed["EMA"]
        ...
        for combination in self.indicator_inputs_required["MACD"] :
            ...
            fast_ema = ema_formula(self.price, combination[0], self.indicator_historical[self.ticker][loading_index])

            #Calculate the live slow EMA value for this combination using the "ema_formula" function 
            slow_ema = ema_formula(self.price, combination[1], self.indicator_historical[self.ticker][loading_index+1])

            #Calculate the live MACD EMA value for this combination using the "ema_formula" function 
            macd_ema = ema_formula(fast_ema-slow_ema, combination[2], self.indicator_historical[self.ticker][loading_index+2])
            ...
            loading_index += self.indicator_data_points_needed["MACD"]
    ...
    def convert_volume_to_integer(self, volume_str:str) :
        ...
        #Convert the letters to integers if the string contains letters
            ...
            number_translations = {"M":1000000, "B":1000000000}
    ...
    def deploy_strategies(self, testing_mode) :
        ...
        while True :
            ...
            for scraped_stock in scraped_data :
                ...
                print(self.ticker, self.price)
                print(self.indicators)
                print(self.stock_statistics)
                print()
                ...
                for strategy_index, strategy in enumerate(self.strategies) :
                    ...
                    if self.ticker in self.strategies_performance_tracking[strategy_index][0] :
                        ...
                        if strategy[2](self.indicators) :

                            #Update "strategies_performance_tracking" list by logging the ticker, starting price, Buy/Short boolean, and current price
                            self.strategies_performance_tracking[strategy_index][1].append((self.ticker)+self.strategies_performance_tracking[strategy_index][0]+(self.price))
